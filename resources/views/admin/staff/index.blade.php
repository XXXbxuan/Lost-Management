                            ...
                            <tbody class="text-gray-600 text-sm font-light">
                                @forelse($staffMembers as $staff)
                                    <tr class="border-b border-gray-200 hover:bg-gray-50">
                                        <td class="py-3 px-6 text-left whitespace-nowrap font-bold">#{{ $staff->staff_id }}</td>
                                        <td class="py-3 px-6 text-left">{{ $staff->name }}<br><span class="text-xs text-gray-400">{{ $staff->department }}</span></td>
                                        ...
                                        <td class="py-3 px-6 text-center">{{ $staff->contact_number ?? '-' }}</td>
                                        <td class="py-3 px-6 text-center">
                                            <span class="{{ $staff->status === 'Active' ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800' }} py-1 px-3 rounded-full text-xs font-bold uppercase">
                                                {{ $staff->status }}
                                            </span>
                                        </td>
                                        <td class="py-3 px-6 text-center">
                                            <div class="flex item-center justify-center space-x-2">
                                                <a href="{{ route('admin.staff.edit', $staff->staff_id) }}" class="text-blue-600 hover:text-blue-900 font-bold hover:underline">Manage</a>
                                            </div>
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="6" class="py-4 text-center text-gray-500">No staff members found.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
