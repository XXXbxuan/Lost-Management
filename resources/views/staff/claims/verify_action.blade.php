        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                    ...
                    <div class="flex flex-col items-center text-center">
                        ...
                        <div class="w-full space-y-4 text-left">
                            ...
                            <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                ...
                                <p class="text-sm font-bold text-slate-600 break-all">{{ $match->lostItem->passenger_email }}</p>
                            </div>
                            ...
                            <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Lost Report Reference</p>
                                <p class="text-sm font-black text-slate-800">#L-{{ $match->lostId }}</p>
                            </div>
                            <div class="col-span-2 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Verification Notes</p>
                            <p class="text-sm text-slate-600 leading-relaxed">{{ $match->notes ?? 'No verification notes.' }}</p>
                        </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-[2.5rem] shadow-xl p-8 border-t-8 border-blue-600 relative">
                    ...
                    <div class="mt-6 aspect-video rounded-3xl overflow-hidden border-4 border-slate-50 shadow-inner mb-6 bg-slate-50 flex items-center justify-center">
                        @if(!empty($match->foundItem->image_path))
                            <img src="{{ asset('storage/' . $match->foundItem->image_path) }}" class="w-full h-full object-cover">
                        @else
                            <span class="text-slate-300 text-sm font-bold uppercase tracking-widest">No Image</span>
                        @endif
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            ...
                            <p class="text-sm font-black text-slate-800">{{ $match->foundItem->category ?? 'N/A' }}</p>
                        </div>

                        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Serial Number</p>
                            <p class="text-sm font-black text-slate-800">{{ $match->foundItem->serial_number ?? 'N/A' }}</p>
                        </div>
                        ...
                        <div class="col-span-2 bg-amber-50 p-5 rounded-2xl border-2 border-amber-200">
                            ...
                            <p class="text-2xl font-black text-amber-900">{{ $match->foundItem->storage_location ?? 'N/A' }}</p>
                        </div>
                    </div>
                </div>
            </div>
