<x-app-layout>
        ...
        <div class="py-8">
            <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
                <div class="mb-7 flex flex-wrap items-start justify-between gap-4">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.35em] text-sky-500">Inventory Slot</p>
                        <h1 class="mt-2 text-4xl font-bold tracking-tight text-slate-900">Slot {{ $slot->slot_code }}</h1>
                    ...
                    <a href="{{ route('staff.inventory.index', ['zone' => $slot->zone_code]) }}"
    ...
    <div>
        ...
        <p class="mt-1 text-sm text-slate-500">Detailed information for the item stored in this slot.</p>
    </div>
    ...
    <div @click.away="showServiceForm = false"
        ...
        @if ($item)
                                    ...
                                    <div>
                                        ...
                                        <select name="new_location"
                                            ...
                                            <template x-for="slotOption in filteredSlots()" :key="slotOption.full_code">
                                                <option :value="slotOption.full_code"
                                                        :disabled="slotOption.disabled"
                                                        x-text="slotOption.label"></option>
                                            </template>
                                        </select>
            ...
            <div x-show="showRemoveForm"
                 x-transition.opacity
                 x-cloak
                 @keydown.escape.window="showRemoveForm = false"
                 class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-6"
                 style="display: none;">
                <div @click.away="showRemoveForm = false"
                     class="w-full max-w-3xl rounded-[2rem] bg-white p-6 shadow-2xl">
                    <div class="flex items-center justify-between border-b border-slate-200 pb-4">
                        <div>
                            <h3 class="text-xl font-bold text-slate-900">Remove Item</h3>
                            <p class="mt-1 text-sm text-slate-500">Remove this item from the inventory slot.</p>
                        </div>
                            ...
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                                ...
                                <div class="mt-4 space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <textarea name="removal_reason"
                                                  rows="4"
                                                  class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm focus:border-slate-500 focus:ring-slate-500"
                                                  placeholder="Enter removal reason..."
                                                  required>{{ old('removal_reason') }}</textarea>
