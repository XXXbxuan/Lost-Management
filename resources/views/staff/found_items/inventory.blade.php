    ...
    <div class="py-8">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="mb-6 flex items-center justify-between">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-[0.35em] text-sky-500">Found Items</p>
                    <h1 class="mt-2 text-3xl font-bold text-slate-900">Inventory Map</h1>
                ...
                <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
                    <div class="flex flex-wrap gap-3">
                        @foreach (['GEN' => 'Place 1', 'VAULT' => 'Place 2', 'BAG' => 'Place 3'] as $zoneCode => $zoneLabel)
                            <a href="{{ route('staff.inventory.index', ['zone' => $zoneCode]) }}"
                                ...
                                <div class="flex items-start justify-center gap-10">
                                    <div class="grid grid-cols-1 gap-3">
                                        @foreach ($leftSlots as $slot)
                                            ...
                                            <a href="{{ route('staff.inventory.show_slot', $slot->full_code) }}"
                                               ...
                                               class="relative flex h-16 w-14 items-center justify-center rounded-t-[18px] rounded-b-[24px] border-2 shadow-md transition duration-150 hover:-translate-y-1 hover:shadow-lg {{ $seatClass }}">
                                                ...
                                                <span class="mt-1 text-[13px] font-extrabold tracking-wide">{{ $slot->slot_code }}</span>
                                            </a>
                                        @endforeach
                                    </div>
