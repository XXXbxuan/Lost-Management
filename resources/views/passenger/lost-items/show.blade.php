        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white p-6 shadow rounded-lg border-t-4 border-red-500">
                <div class="flex justify-between items-center border-b pb-4 mb-4">
                    <h3 class="font-bold text-lg text-gray-800">Report #{{ $lostReport->id }}</h3>
                    <span class="px-3 py-1 rounded-full text-xs font-bold uppercase 
                        {{ $lostReport->status == 'Matched' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600' }}">
                        {{ $lostReport->status }}
                    </span>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 text-sm">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <span class="font-bold text-gray-900 text-lg">{{ $lostReport->item_name }}</span>
                        </div>
                        ...
                        <div>
                            ...
                            @if($lostReport->flight_number)
                                <span class="text-xs text-blue-500 ml-2">(Flight: {{ $lostReport->flight_number }})</span>
                            @endif
                        </div>
                        <div>
                            <span class="block text-xs font-bold text-gray-500 uppercase">Date Lost</span>
                            <span class="text-gray-900">{{ $lostReport->lost_time->format('d M Y, h:i A') }}</span>
                        </div>
                        <div>
                            <span class="block text-xs font-bold text-gray-500 uppercase">Description</span>
                            <p class="text-gray-600 bg-gray-50 p-3 rounded mt-1 border">{{ $lostReport->description ?? 'No description provided.' }}</p>
                        </div>
                    </div>
                </div>
