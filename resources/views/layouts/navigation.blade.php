<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(Auth::user()->role === 'Passenger')
                        <x-nav-link :href="route('passenger.report')" :active="request()->routeIs('passenger.report')">
                            {{ __('Report Lost Item') }}
                        </x-nav-link>
                        <x-nav-link :href="route('passenger.found_items')" :active="request()->routeIs('passenger.found_items')">
                            {{ __('Browse Found Items') }}
                        </x-nav-link>
                        <x-nav-link :href="route('passenger.rewards')" :active="request()->routeIs('passenger.rewards')">
                            {{ __('My Rewards') }}
                        </x-nav-link>
                    @endif
                    ...
                    @if(Auth::user()->role === 'Staff' || Auth::user()->role === 'Admin')
                        ...
                        <x-nav-link :href="route('staff.found-items.index')" :active="request()->routeIs('staff.found-items.*')">
                            {{ __('Found Items') }}
                        </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(Auth::user()->role === 'Passenger')
                <x-responsive-nav-link :href="route('passenger.report')" :active="request()->routeIs('passenger.report')">
                    {{ __('Report Lost Item') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('passenger.found_items')" :active="request()->routeIs('passenger.found_items')">
                    {{ __('Browse Found Items') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('passenger.rewards')" :active="request()->routeIs('passenger.rewards')">
                    {{ __('My Rewards') }}
                </x-responsive-nav-link>
            @endif
