<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="md:col-span-2">
        ...
        <select name="color" id="mainColorSelect"
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                required>
            ...
            <option value="Multi-color" {{ old('color') == 'Multi-color' ? 'selected' : '' }}>
                ...
                @foreach(['Black','White','Red','Blue','Green','Yellow','Orange','Purple','Pink','Grey','Brown','Silver','Gold'] as $subColor)
                    <div class="flex items-center">
                        <input
                            ...
                            {{ in_array($subColor, old('sub_colors', [])) ? 'checked' : '' }}
                        >
...
<script>
    // Multi-color UI (component-level, no dependency on parent page)
    document.addEventListener('DOMContentLoaded', function () {
        const sel = document.getElementById('mainColorSelect');
        const box = document.getElementById('multiColorOptions');

        function sync() {
            if (!sel || !box) return;
            if (sel.value === 'Multi-color') box.classList.remove('hidden');
            else box.classList.add('hidden');
        }

        if (sel) sel.addEventListener('change', sync);
        sync(); // run once for old() / refresh
    });
</script>